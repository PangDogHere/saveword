...
# 设置要爬取的页面URL
urls = [
    'https://nanchong.lianjia.com/ershoufang/jialingqu/',
    'https://nanchong.lianjia.com/ershoufang/yilongxian/',
    'https://nanchong.lianjia.com/ershoufang/nanbuxian/',    
    'https://nanchong.lianjia.com/ershoufang/yingshanxian/',
    'https://nanchong.lianjia.com/ershoufang/penganxian/',
    'https://nanchong.lianjia.com/ershoufang/xichongxian/',
    'https://nanchong.lianjia.com/ershoufang/langzhongshi/',
    'https://nanchong.lianjia.com/ershoufang/shunqingqu/',
    'https://nanchong.lianjia.com/ershoufang/gaopingqu/'
]
# 循环读取每个页面的信息
data = []
for url in urls:    
    ...
    while True:        
        ...
        # 获取所有房屋信息
        houses = soup.select('.clear.LOGCLICKDATA')
        for house in houses:
            if house.select_one('.positionInfo') is not None:
                # 获取小区名
                if house.select_one('.positionInfo') is not None:
                    name = house.select_one('.positionInfo').contents[2].text.strip()
                    address = house.select_one('.positionInfo').contents[4].text.strip()
                else:
                    name = "无"
                    address = "无"
                # 获取地址
        ...
        # 查找下一页按钮
        next_page = soup.select_one('.page-box.fr')
        if next_page.text.find('下一页') == -1:
            ...
            if page_data['curPage'] < page_data['totalPage']:
                url = '{}pg{}'.format(url_Tmp,str(page_data['curPage'] +1))
